...
"""
UserProxyAgent-(Humanuser) + 
AssistantAgent(Researcher, reviewer) + GroupChatManager example using AutoGen framework."""
import autogen
from autogen import (
    AssistantAgent,
    UserProxyAgent,
...
def build_agents():
    ...
    user_proxy = UserProxyAgent(
        name="HumanUser",
        human_input_mode="NEVER",#ALWAYS, TERMINATE, #AUTO
    ...
    composer = AssistantAgent(
        name="composer",
        llm_config=LLM_CONFIG,
        system_message=(
            "You are a professional composer.\n"
            "take the reviewed content and make it presentation ready.\n"
        "- End with a line: 'FINAL_DECISION: APPROVED'."),
        description="Composes and polishes the answer, then approves.",
    )
    ...
    analyst = AssistantAgent(
        ...
        system_message=(
            "You are an analyst.\n"
            "- Analyze the draft provided by the Researcher.\n"
            "- Identify gaps, inconsistencies, and areas for improvement.\n"
            "- Provide a detailed analysis report."
        ),
        description="Analyzes the draft and identifies areas for improvement."
    )
    ...
    strategist = AssistantAgent(
        ...
        system_message=(
            ...
            "- Review the analysis report from the Analyst.\n"
            "- Suggest actionable strategies to address the identified gaps.\n"
            "- Ensure the strategies are practical and aligned with the user’s goals."
        ),
        description="Provides actionable strategies based on the analysis."
    )

    group_chat = GroupChat(
        agents=[user_proxy, researcher, reviewer, composer, analyst, strategist],
        messages=[],
        max_round=12,
        speaker_selection_method="auto",#ROUND_ROBIN--->U--R---REV---U
...
def run_multi_agent_workflow(user_topic: str, extra_instruction: str):
    ...
    message = f"""
            ...
            2. Reviewer: improves.
            3. Composer: makes it presentation ready.
            4. end with 'FINAL_DECISION: APPROVED'.
    ...
    for m in reversed(msgs):
        if m.get("name") == "composer" and "FINAL_DECISION: APPROVED" in m.get("content", ""):
